...
import { OPDUtilsParsingError } from './Errors';
...
export class EnclosingPair {
	...
	constructor(openingString: string, closingString?: string) {
		if (isFalsy(openingString)) {
			throw new OPDUtilsParsingError('opening string must not be empty');
		}
...
export class ParserUtils {
	...
	static split(str: string, separator: string, ignore?: EnclosingPair): string[] {
		if (!str) {
			throw new OPDUtilsParsingError('string must not be empty');
		}
		if (!separator) {
			throw new OPDUtilsParsingError('separator must not be empty');
		}
	...
	static isStringAt(str: string, subStr: string, index: number): boolean {
		if (index < 0) {
			throw new OPDUtilsParsingError('index must be greater than 0');
		}
		if (index >= str.length) {
			throw new OPDUtilsParsingError('index out of bounds of string');
		}
		if (!str) {
			throw new OPDUtilsParsingError('string must not be empty');
		}
		if (!subStr) {
			throw new OPDUtilsParsingError('sub string must not be empty');
		}
